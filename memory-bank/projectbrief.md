...
- **Генератор сигналов:** фоновое обновление holding‑регистров по заданному сигналу (синус, пила, меандр, константа) в форматах INT16/FLOAT32/FLOAT64; конфигурация привязана к профилям; в GUI — панель под таблицей регистров, мини‑график сигнала в реальном времени (живые данные), отображение значения, неоновая подсветка (цвет настраивается), подсветка затрагиваемых ячеек регистров; автообновление значений в интерфейсе.
- Docker Compose, файловый storage (config/state, profiles с полем generators).
- **Тесты (164+ теста):**
  - Backend: 94 теста (API endpoints, encoding_utils, **полное покрытие Modbus FC01-FC06/FC15-FC16**)
  - Frontend: 70+ тестов (converters для всех форматов, hooks)
