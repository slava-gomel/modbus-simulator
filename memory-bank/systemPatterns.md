...
- WEB‑GUI организован как тёмный «дешборд»:
  ...
  - регистры отображаются таблицей диапазонов; ячейки, затрагиваемые включённым генератором, подсвечиваются неоном (цвет настраивается у генератора); при Modbus‑записях (по данным из `/api/server/modbus_log`) для затронутых адресов holding и `coils` кратковременно включается зелёная анимация вспышки;
